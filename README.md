...
Silicon Macs. There is also an experimental Apple TV preview. The current alpha
release is **0.1.0-alpha94**.
...
- Offline downloads for individual movies and episodes on Android, iOS, and
  macOS when content downloading is enabled for the Jellyfin account
...
or newer and is unsupported, with no hardware-validation, feature-parity, or
ongoing-support commitment. The macOS package requires macOS 13 or newer on Apple Silicon;
