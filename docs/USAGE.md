...
opens the **Switch account** dialog.
...
filter. Changing watched state can also change whether a title appears in
**Continue Watching**.
...
suggestions, and upcoming episodes on those layouts. The unsupported Apple TV
preview has a simpler native Home, Libraries, Search, and Settings shell; it
does not expose the library controls, Find filters, or Discover flow described
above.

### Choose a media version

On phones, tablets, macOS, and Android TV, a movie or episode with at
...
choice on the concrete episode where it is offered. The native Apple TV preview
does not expose this version picker.

## Download for offline playback

Downloads are available on Android phones, tablets, and TVs, iPhone, iPad, and
macOS when the server grants the account's **Content downloading** permission.
...
1. Open **Downloads** and choose **Manage allocated size** on touch layouts or
   **Manage allocation** on Android TV.
...
this device.
When connected to the matching account, downloaded playback can also make
best-effort live server reports. Missed reports are not automatically replayed
or synchronized when connectivity returns.

On iPhone and iPad, offline playback requires VLCKit for that session even when
AVPlayer is the saved online default. This does not change the saved default.
...
resume the paused item. Opening the Downloads screen wakes eligible queued
work, but does not override an explicit pause. Android can schedule work with
the operating system. On iOS, downloading advances while JellyScope is active;
...
is present. These TV layouts do not offer the per-account removal action.
...
| Apple TV preview | System player | Best-effort preview playback | Unsupported preview; no hardware-validation commitment |
...
and exits the player. The unsupported Apple TV preview uses its native system
player, and this guide does not certify PiP behavior there.
...
| AVPlayer or VLCKit (iOS), LibVLC (macOS), Apple TV system player | Unavailable | Unavailable |
...
2. On phones, tablets, and macOS, optionally set **Subtitle result preference**.
   ...
   TV does not expose this preference control.
...
account removal, and sign-out until cleared. The native Apple TV preview does
not expose this OpenSubtitles search, install, or clear workflow.

## Queue, chapters, skipping, and autoplay

- In a movie library, **Shuffle All** creates a playback queue from the current
  filtered results. A normal standalone item with no playlist need not expose a
  queue.
...
- On Android TV, press Down from the controls to open the **Up Next**
  ...
  offer **Shuffle queue**. The native Apple TV preview does not expose an app
  queue browser or shuffle control.
...
The Apple TV preview uses native AVKit and SwiftUI controls. The Android TV key
mappings in this table do not describe or certify Siri Remote behavior.
