...
- Shared and TV logged-in roots are keyed by identity plus epoch, so same-account
  token rotation receives a fresh lifecycle. `accounts` and `SessionState` derive
  from one committed snapshot, including active markers.
...
tvos-app/                 # native SwiftUI/AVKit shell
```
...
- iOS wires AVPlayer, settings/session storage, and Apple lifecycle. App
...
- tvOS exposes only `TvosEntry` to Swift for Koin, presenter factories, and dev
  prefill. Apple-family implementations live in `appleMain`; database directory
  and DI entry modules are target-specific. tvOS Caches storage is purgeable;
  hardware playback/display/remote acceptance cannot be inferred from simulator.

### Downloads module boundary

Android mobile/TV, iOS, and desktop opt into `downloadsModule` and supply their
database, artifact store, and lifecycle/execution host. tvOS installs none and
makes no feature claim. Repositories/adapters feed domain UseCases/Actions,
ViewModels consume those surfaces, and UI never owns queue/filesystem. The
...
- Native tvOS preserves Siri Remote focus and keeps SwiftUI/AVKit outside common
  ...
  stay in Apple source sets; only storage directory and DI entry remain per target.
- Downloads is installed only on platforms that supply durable storage and
  lifecycle execution, avoiding a false tvOS obligation and cache-owned media.
