...
- **tvOS retains the system player.** `AVPlayerViewController` provides native
  chapters, segment skipping, and track menus; a custom transport overlay would
  duplicate system ownership. Simulator results remain insufficient evidence
  for Apple TV decoding, PiP, remote interaction, or presentation.
